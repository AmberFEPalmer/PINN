...
### Transformer model for 1, 2, 3 and 4 week predictions
...
class transformer_time_series(nn.Module):
    def __init__(self, input_size=3, d_model=d_model, nhead=nhead, num_layers=num_layers, window_size=window_size):
        super().__init__()
...
def train_one_step(scaled):
    ...
    model = transformer_time_series().to(device)
...
print("\ntransformer mase by horizon (model mae / naive mae; <1 beats naive):")
fig, axes = plt.subplots(3, 1, figsize=(12, 11), sharex=True)
for ax, s in zip(axes, series):
    hres = collect(s)
    ax.plot(study_dates, data[s] * n_val, color="#004F94", lw=1.3, ls=":", label="observed")
    for h in range(1, 5):
    ...
    for h in range(1, 5):
        ...
        line += f"  {h}wk={mase:.3f}"
    print(line)
axes[0].set_title("england: transformer per-horizon vs observed")
plt.tight_layout(); plt.savefig("transformer_perhorizon.png", dpi=150); plt.show()
...
for h, ax in zip(range(1, 5), axes.flatten()):
    ...
    if hd:
        ...
        ax.plot(hd, hp, color="#bc7ff6", lw=1.5, label=f"transformer {h}-week")
...
fig.suptitle("transformer: cases (1-4 weeks ahead)", fontsize=14)
plt.tight_layout(); plt.savefig("transformer_cases_grid.png", dpi=150); plt.show()

### Save
import os, pickle
os.makedirs("../../forecasts", exist_ok=True)
with open("../../forecasts/preds_transformer.pkl", "wb") as f:
