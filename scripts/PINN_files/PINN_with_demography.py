...
S0 = tf.constant(67000000/67000001, dtype=tf.float32)
E0 = tf.constant(0.0, dtype=tf.float32)
I0 = tf.constant(1/67000001, dtype=tf.float32)
...
def loss_function(t_col, t_data_loss, I_data_loss, net):
    ...
    b = tf.constant(0.0000286, dtype=tf.float32, name='births')
    d = tf.constant(0.0000273, dtype=tf.float32, name='deaths')
    N = tf.constant(7000000, dtype=tf.float32, name='population')
    ...
    dS_dt_physics = T * (b - beta * S * I - d * S)
    dE_dt_physics = T * (beta * S * I - sigma * E - d * E)
    dI_dt_physics = T * (sigma * E - (gamma + d) * I)
    dR_dt_physics = T * (gamma * I - d * R)
