...
def create_pinn_model():
    ...
    raw = Dense(4, activation=None)(seir)
    outputs = tf.keras.layers.Softmax()(raw)
    S = outputs[:, 0:1]
    E = outputs[:, 1:2]
    I = outputs[:, 2:3]
    R = outputs[:, 3:4]
...
def loss_function(t_col, t_data_loss, I_data_loss, net):
    ...
    beta = tf.constant(0.8, dtype=tf.float32, name='beta')
...
for itr in range(5000):
    t_col_uniform = np.random.uniform(0, 1, n_collocation).reshape(-1, 1)
    t_col_tensor = tf.convert_to_tensor(t_col_uniform, dtype=tf.float32)
