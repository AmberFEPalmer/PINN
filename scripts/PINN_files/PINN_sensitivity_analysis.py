...
def create_pinn_model(n_neurons=50, n_layers=3, l2_coef=1e-5):
    t_input = Input(shape=(1,), name='time_input')

    ### hidden layers compartmental model
    ### tanh activation for nonlinearity, L2 regularisation to prevent overfitting
    core = t_input
    for _ in range(n_layers):
        core = Dense(n_neurons, activation='tanh',
                      kernel_regularizer=regularizers.l2(l2_coef))(core)
    ...
    ### hidden beta subnetwork, same depth/width as SEIR net, tanh activation, L2 regularisation
    beta_h = t_input
    for _ in range(n_layers):
        beta_h = Dense(n_neurons, activation='tanh',
                        kernel_regularizer=regularizers.l2(l2_coef))(beta_h)
...
def compute_loss(t_col, t_data, I_d, H_d, D_d, net, logit_p_h, logit_p_d,
                 total_weeks, L0, R0, w_ode=0.1, w_ic=0.1):
    ...
    total = 1.0 * data_loss + w_ode * ode_loss + w_ic * ic_loss
...
def train_window(t_train_norm, I_tr, H_tr, D_tr, total_weeks, inc0, R0,
                 ...
                 n_neurons=50, n_layers=3, l2_coef=1e-5,
                 w_ode=0.1, w_ic=0.1, n_colloc=500):
    model = create_pinn_model(n_neurons=n_neurons, n_layers=n_layers, l2_coef=l2_coef)
    ...
    t_col = tf.convert_to_tensor(np.linspace(0, 1.0, n_colloc).reshape(-1, 1), dtype=tf.float32)
...
### SENSITIVITY SWEEP BLOCK 
if SWEEP_MODE:
    sweep_configs = [
        {"name": "baseline",      "n_neurons": 50,  "n_layers": 3, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 500},
        {"name": "w_ode_low",     "n_neurons": 50,  "n_layers": 3, "l2_coef": 1e-5, "w_ode": 0.01, "n_colloc": 500},
        {"name": "w_ode_high",    "n_neurons": 50,  "n_layers": 3, "l2_coef": 1e-5, "w_ode": 1.0,  "n_colloc": 500},
        {"name": "narrow",        "n_neurons": 30,  "n_layers": 3, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 500},
        {"name": "wide",          "n_neurons": 100, "n_layers": 3, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 500},
        {"name": "shallow",       "n_neurons": 50,  "n_layers": 2, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 500},
        {"name": "deep",          "n_neurons": 50,  "n_layers": 4, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 500},
        {"name": "l2_off",        "n_neurons": 50,  "n_layers": 3, "l2_coef": 0.0,  "w_ode": 0.1, "n_colloc": 500},
        {"name": "l2_strong",     "n_neurons": 50,  "n_layers": 3, "l2_coef": 1e-3, "w_ode": 0.1, "n_colloc": 500},
        {"name": "colloc_sparse", "n_neurons": 50,  "n_layers": 3, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 100},
        {"name": "colloc_dense",  "n_neurons": 50,  "n_layers": 3, "l2_coef": 1e-5, "w_ode": 0.1, "n_colloc": 2000},
    ]

    total_weeks = float(N_total_points - 1)
    os.makedirs("sensitivity_outputs", exist_ok=True)

    sweep_summary = []       # one row per config: aggregated across all windows
    sweep_per_window = []    # one row per (config, window): full detail for robustness checks

    for cfg in sweep_configs:
        print(f"\n{'='*60}\nSweep config: {cfg['name']} -- running ALL windows\n{'='*60}")
        tf.random.set_seed(42)  # fixed seed across configs -- isolates the hyperparameter effect

        warm = None
        config_case_mase_all = []   # per-window MASE, collected then aggregated
        ...
        for train_end in range(First_train_weeks, N_total_points - Forecast_horizon + 1):
            model, lph, lpd, lL0 = train_window(
                ...
                total_weeks=total_weeks, inc0=inc0, R0=R0,
                n_iter=SWEEP_N_ITER, warm=warm,
                n_neurons=cfg["n_neurons"], n_layers=cfg["n_layers"], l2_coef=cfg["l2_coef"],
                w_ode=cfg["w_ode"], n_colloc=cfg["n_colloc"])
            warm = (model.get_weights(), float(lph.numpy()), float(lpd.numpy()), float(lL0.numpy()))

            origin = train_end - 1

            ### 1-4 week ahead forecasts + MASE (cases), same evaluation as your main pipeline
            case_maes, case_naive_maes = [], []
            for h in range(1, Forecast_horizon + 1):
                fidx = origin + h
                if fidx >= N_total_points:
                    continue
                t_fc = tf.constant([[float(t_data_norm[fidx, 0])]], dtype=tf.float32)
                _, _, _, _, Z, _, _, _, beta = model(t_fc)
                Z_v = float(np.clip(Z.numpy()[0, 0], 0.0, 1.0))
                case_pred = max(0.0, GAMMA_ZW * Z_v)
                case_obs = float(I_data[fidx, 0])
                case_naive = float(I_data[origin, 0])
                case_maes.append(abs(case_pred - case_obs))
                case_naive_maes.append(abs(case_naive - case_obs))
            window_mase = (np.mean(case_maes) / (np.mean(case_naive_maes) + 1e-12)) if case_maes else np.nan
            config_case_mase_all.append(window_mase)

            ### R(t) plausibility check over this window
            t_check = tf.convert_to_tensor(
                np.linspace(0, float(t_data_norm[train_end - 1, 0]), 50).reshape(-1, 1), dtype=tf.float32)
            _, _, _, _, _, _, _, _, beta_check = model(t_check)
            rt_vals = beta_check.numpy().flatten() / GAMMA
            rt_min, rt_max = float(rt_vals.min()), float(rt_vals.max())

            sweep_per_window.append({
                "config": cfg["name"], "train_end": train_end,
                "window_mase": window_mase, "rt_min": rt_min, "rt_max": rt_max,
            })

            if train_end % 10 == 0:
                print(f"  [{cfg['name']}] window train_end={train_end:3d} | "
                      f"MASE={window_mase:.3f} | R(t) range=[{rt_min:.2f}, {rt_max:.2f}]")

        ### aggregate across all windows for this config
        overall_mase = float(np.nanmean(config_case_mase_all))
        sweep_summary.append({
            **cfg,
            "cases_mase_all_windows": overall_mase,
            "cases_mase_std_across_windows": float(np.nanstd(config_case_mase_all)),
            "n_windows": len(config_case_mase_all),
        })
        print(f"\n>>> Config '{cfg['name']}' complete: "
              f"mean MASE across {len(config_case_mase_all)} windows = {overall_mase:.3f} "
              f"(std {np.nanstd(config_case_mase_all):.3f})")

        ### save per-window detail incrementally, in case a later config run is interrupted
        pd.DataFrame(sweep_per_window).to_csv(
            "sensitivity_outputs/national_pinn_sweep_per_window.csv", index=False)
        pd.DataFrame(sweep_summary).to_csv(
            "sensitivity_outputs/national_pinn_sweep_results.csv", index=False)

    sweep_df = pd.DataFrame(sweep_summary)
    print("\n=== Full sweep results (aggregated across all windows) ===")
    print(sweep_df.sort_values("cases_mase_all_windows").to_string(index=False))

    ### Plot: one panel per swept dimension, MASE as the headline metric
    fig, axes = plt.subplots(2, 2, figsize=(12, 9))

    def bar_panel(ax, names, values, ylabel, title):
        ax.bar(names, values, color="#457b9d")
        ax.set_ylabel(ylabel)
        ax.set_title(title)
        ax.tick_params(axis='x', rotation=30)
        ax.grid(axis='y', alpha=0.3)

    sub = sweep_df[sweep_df["name"].isin(["w_ode_low", "baseline", "w_ode_high"])]
    bar_panel(axes[0, 0], sub["name"], sub["cases_mase_all_windows"], "cases MASE (all windows)", "Effect of ODE loss weight")

    sub = sweep_df[sweep_df["name"].isin(["narrow", "baseline", "wide", "shallow", "deep"])]
    bar_panel(axes[0, 1], sub["name"], sub["cases_mase_all_windows"], "cases MASE (all windows)", "Effect of network width/depth")

    sub = sweep_df[sweep_df["name"].isin(["l2_off", "baseline", "l2_strong"])]
    bar_panel(axes[1, 0], sub["name"], sub["cases_mase_all_windows"], "cases MASE (all windows)", "Effect of L2 regularisation")

    sub = sweep_df[sweep_df["name"].isin(["colloc_sparse", "baseline", "colloc_dense"])]
    bar_panel(axes[1, 1], sub["name"], sub["cases_mase_all_windows"], "cases MASE (all windows)", "Effect of collocation density")

    plt.tight_layout()
    plt.savefig("sensitivity_outputs/national_pinn_sweep_mase.png", dpi=150)
    plt.show()

    print("\nSweep complete. Set SWEEP_MODE = False and re-run to use the full rolling-window pipeline below.")
