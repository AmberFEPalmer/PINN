...
def create_pinn_model():
    ...
    gamma = Dense(1, activation=None, name='gamma')(gamma_hidden)

    model = Model(inputs=t_input, outputs=[S, E, I, R, beta, gamma])
    ...
    sigma = Dense(1, activation=None, name='sigma')(sigma_hidden)
...
def loss_function(t_col, t_data_loss, I_data_loss, net):
    ...
    d_gamma_dt = tape.gradient(gamma, t_col)
    gamma_smooth_loss = tf.reduce_mean(tf.square(d_gamma_dt))
    
    d_sigma_dt = tape.gradient(sigma, t_col)
    sigma_smooth_loss = tf.reduce_mean(tf.square(d_sigma_dt))
