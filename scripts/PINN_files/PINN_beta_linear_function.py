...
### Plot: PINN predictions vs observed
...
plt.title('PINN observed vs predicted — time-varying β (90/10 split)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'PINN_tv_beta_I_pred.png'), dpi=150)
plt.show()

### Plot:Estimated β(t) vs true β(t)
...
plt.xlabel('Normalised time')
plt.ylabel('β(t)')
plt.title('Estimated β(t) vs true β(t) — time-varying PINN (90/10 split)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'PINN_tv_beta_est.png'), dpi=150)
plt.show()
