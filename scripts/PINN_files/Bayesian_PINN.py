...
### Load CSV 
data_path = os.path.join(data_folder, "SEIR_data_beta_0.5.csv")
...
t_max_val = t_data.max()
t_data  = t_data  / t_max_val
t_train = t_train / t_max_val
t_test  = t_test  / t_max_val
...
log_sigma_obs = tf.Variable(0.0, trainable=True, dtype=tf.float32)
...
def loss_function(t_col, t_data_loss, I_data_loss, net, I_scale):
    ...
    with tf.GradientTape(persistent=True) as tape:
        tape.watch(t_col)
        S, E, I, R, beta = net(t_col, training=True)
    ...
    ### SEIR equations 
    T = tf.constant(t_max_val, dtype=tf.float32)
    ...
    S_0, E_0, I_0, R_0, _ = net(t_zero, training=True)
    ...
    _, _, I_pred, _, _ = net(t_data_normalized, training=True)
    sigma_obs = tf.clip_by_value(tf.exp(log_sigma_obs), 1e-3, 0.1)

    dist = tfd.Normal(loc=I_pred, scale=sigma_obs)
    log_likelihood = tf.reduce_mean(dist.log_prob(I_data_loss))
    data_loss = -log_likelihood

    
    ### Total loss
    N_data = t_train.shape[0]
    Kl_loss = tf.add_n(net.losses) / N_data
    total_loss = 1.0 * data_loss + 1.0 * Initial_condition_loss + 1.0 * conservation_loss + 0.1*ODE_loss + (kl_weight_var * Kl_loss) + 0.1*beta_smooth_loss
...
trainable_vars = model.trainable_variables + [log_sigma_obs]
...
def predict_with_uncertainty(model, t, n_samples=1000):
    ...
    for _ in range(n_samples):
        S, E, I, R, beta = model(t, training=True)
...
for itr in range(total_iters):
    ...
    if itr % 10000 == 0:
        ...
        KL_scaled = kl_weight_var * (KL_raw)
        print(
...
mean, std = predict_with_uncertainty(model, t_tensor, n_samples=1000)
...
plt.xlabel('Time')
plt.ylabel('Infected (normalized)')
plt.title('SEIR Bayesian PINN β = 0.5 90/10 test/train split')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'Bayesian_PINN_beta_0.5_90_10.png'))
plt.show()
...
for _ in range(n_samples):
    _, _, _, _, beta = model(t_plot_tensor, training=True)
...
plt.savefig('B-PINN_param_est_Beta_0.5.png', dpi=300)
plt.show()